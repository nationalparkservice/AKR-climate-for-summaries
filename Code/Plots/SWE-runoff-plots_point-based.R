...
# runoff$X <- NULL
# 
# SWE$name = "SWE"  # ← Not useful here
# runoff$name = "runoff"
# names(SWE)[2] = "var"  # ← Irrelevant
# names(runoff)[2] = "var"
...
df$date <- format(as.Date(df$time, format = "%m/%d/%Y"), "%Y-%m-%d")
df$Year <- as.factor(substr(df$date, 1, 4))
df$yday <- lubridate::yday(as.Date(df$date))
...
# Updates from ACR for parks with no snow off days
# snow.on <- df %>%
