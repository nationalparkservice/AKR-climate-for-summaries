...
for(i in 1:length(grid_filelist)){
  ...
  DF = rbind(DF, df)
  rm(annual_thresholds,freeze.thaw.sum, GDD.sum,under32.sum,over20.sum,WSF.below32,
     W.under32,pcp.over.5.sum,grid_threshold,grid_crop)
  gc()
}
...
for (G in 1:length(GCMs)){
  ...
  for(i in 1:length(fut_filelist)){
    ...
    annual_thresholds <- c(freeze.thaw.sum, GDD.sum,under32.sum,over20.sum,WSF.below32,
                           W.under32,pcp.over.5.sum )
    ...
    rm(annual_thresholds,freeze.thaw.sum, GDD.sum,under32.sum,over20.sum,WSF.below32,
       W.under32,pcp.over.5.sum,fut_threshold,fut_crop)
    gc()
  }
}
