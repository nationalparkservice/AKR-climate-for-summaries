...
working.dir <- paste0("C:/Users/brobb/OneDrive - DOI/Projects/AKR_CFs/AKR-CFs/",SiteID)
...
buff_shp <- st_buffer(shp, 50000)


## met-daily-stars_by_year
...
for(i in 1:length(grid_filelist)){
  ...
  if (all(is.na(s$freeze.thaw)) & all(is.na(s$GDD)) & all(is.na(s$under32))
      & all(is.na(s$over20.sum)) & all(is.na(s$WSF.below32)) & all(is.na(s$W.under32))
      & all(is.na(s$pcp.over.5.sum))) {
    next  # skip to next file/year
  }
...
for (G in 1:length(GCMs)){
  ...
  for(i in 1:length(fut_filelist)){
    ...
    # # If all values are NA, fallback to nearest grid cell
    # if (all(is.na(fut_crop$tmin)) & all(is.na(fut_crop$tmax)) & all(is.na(fut_crop$pcp))) {
    #   message("No overlap found — using nearest grid cell instead")
    #   pt <- st_centroid(shp)
    #   pt <- st_transform(pt, st_crs(fut_star))
    #   fut_crop <- st_extract(fut_star, at = pt)
    # }
    # shp <- st_transform(shp, st_crs(fut_star))
    # buff_shp <- st_buffer(shp, 50000)
